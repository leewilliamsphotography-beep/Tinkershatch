...
const SplashModule = (function () {
  function init() {
    ...
    function closeSplash() {
      ...
      // Remember that they have visited so we don't show it next time
      safeSet('th-visited', 'true');
    }

    // If they are a returning visitor, skip the splash screen entirely!
    if (hasVisited === 'true') {
      splash.classList.add('hidden');
      document.body.style.overflow = 'auto';
    } else {
      // First time visiting: lock the screen and show the splash
...
const QuickJumpModule=(function(){
    function init(){
        const t=document.getElementById('quickJumpToggle'),c=document.getElementById('quickJumpContent');
        if(!t||!c)return;
        ...
        document.addEventListener('click',e=>{
            if(c.classList.contains('show')&&!c.contains(e.target)&&!t.contains(e.target)){
                c.classList.remove('show');
                t.setAttribute('aria-expanded','false');
            }
        });

        c.querySelectorAll('.quick-jump-link').forEach(link=>{
        ...
        const footerLink=document.getElementById('footerChangelogLink');
        const changelogSection=document.getElementById('changelog');
        if(footerLink&&changelogSection){
            footerLink.addEventListener('click',e=>{
                e.preventDefault();
                changelogSection.style.display='block';
                setTimeout(()=>changelogSection.scrollIntoView({behavior:'smooth'}),100);
            });
        }
    }
    return{init};
...
const HapticModule=(function(){
    function init(){
        // Add subtle vibration to important clicks on mobile devices
        document.body.addEventListener('click', e => {
            if (navigator.vibrate && e.target.closest('button, .mood-btn, .ambient-btn, .gallery-thumb, .splash-shortcut')) {
                navigator.vibrate(8); // Very subtle 8ms tap
            }
        }, { passive: true });
    }
    return { init };
...
const BionicModule=(function(){
    function toggle(state){
        const b=document.body;
        if(state==='on') b.classList.add('bionic-mode');
        else b.classList.remove('bionic-mode');
        safeSet('th-bionic',state);
        
        document.querySelectorAll('.bionic-btn').forEach(btn=>{
            btn.setAttribute('aria-pressed',btn.dataset.bionic===state);
        });
    ...
    function applyBionic(){
        const elements=document.querySelectorAll('#main p, #main h1, #main h2, #main h3, #main li, #main summary');
        elements.forEach(el=>{
            if(el.dataset.bioniced) return; // Skip if already done
            // Simple regex to bold first half of words, ignoring HTML tags
            let html=el.innerHTML;
            if(html.includes('<')) return; // Skip complex elements with nested HTML for safety
            ...
            words.forEach(word=>{
                if(word.trim().length>1){
                    ...
                    newHtml+=`<b>${word.substring(0,half)}</b>${word.substring(half)}`;
                }else{
                    newHtml+=word;
                }
            });
            el.innerHTML=newHtml;
            el.dataset.bioniced='true';
        });
    }

    function removeBionic(){
        const elements=document.querySelectorAll('[data-bioniced="true"]');
        elements.forEach(el=>{
            // Find <b> tags we added and unwrap them
            el.querySelectorAll('b').forEach(b=>{
                const text=document.createTextNode(b.textContent);
                b.parentNode.replaceChild(text,b);
            });
            // Normalize merges adjacent text nodes back together cleanly
            el.normalize();
            delete el.dataset.bioniced;
        });
    }

    function init(){
        const btns=document.querySelectorAll('.bionic-btn');
        if(!btns.length)return;
        let s=safeGet('th-bionic')||'off';
        if(s==='on'){
            // Wait for dynamic content to load before applying
            setTimeout(()=>toggle('on'),1000);
        }
        btns.forEach(btn=>btn.addEventListener('click',()=>toggle(btn.dataset.bionic)));
    }
    return{init};
})();

const NextSectionModule=(function(){
    let sections=[];
    ...
    function init(){
        ...
        sections=Array.from(navDots).map(dot=>{
        ...
        function update(){
            ...
            for(let i=0;i<sections.length;i++){
                if(sections[i].offsetTop<=scrollPos){
                    if(i<sections.length-1&&sections[i+1].offsetTop>scrollPos){
                        nextIndex=i+1;
                        break;
                    }
                }
            ...
            if(nextIndex!==-1){
                ...
                const title=nextSection.querySelector('h2')?.innerText||nextSection.id.replace(/-/g,' ');
                btn.innerHTML=`<span>Next: ${title}</span> <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3"><path d="M12 5v14M5 12l7 7 7-7"/></svg>`;
                btn.classList.add('show');
                btn.onclick=()=>nextSection.scrollIntoView({behavior:'smooth'});
            }else{
...
const WeatherModule=(function(){
    async function init(){
        ...
        try {
            ...
            let icon="🌤️";
            let title="A lovely day";
            let message="A light jacket might be a good idea for a walk.";
            if (code === 0) {
                icon="☀️"; title="Sunny skies!"; message="The sun is shining! Don't forget your suncream and a hat for the garden.";
            } else if (code >= 1 && code <= 3) {
                icon="⛅"; title="Soft clouds"; message="A gentle, overcast day. Perfect for a stroll without getting too hot.";
            } else if (code >= 45 && code <= 48) {
                icon="🌫️"; title="Misty morning"; message="It's a bit foggy out. Take care if you're driving to see us.";
            } else if ((code >= 51 && code <= 67) || (code >= 80 && code <= 82)) {
                icon="🌧️"; title="Rainy day"; message="It's raining! Best bring a coat and a brolly—perfect weather for a cuppa indoors.";
            } else if ((code >= 71 && code <= 77) || (code >= 85 && code <= 86)) {
                icon="❄️"; title="Snowing!"; message="It's snowing! Wrap up warm in your cosiest scarf and mittens.";
            } else if (code >= 95) {
                icon="⛈️"; title="Stormy weather"; message="Stormy out there today. Best to stay cozy by the fire indoors.";
            }
            widget.innerHTML=`
            ...
            widget.innerHTML=`<div class="weather-text"><h3>Weather</h3><p>Check the forecast before visiting!</p></div>`;
        }
    }
    return{init};
})();
const CelebrationModule=(function(){
    async function loadCelebrations(){
        const c=document.getElementById('celebrationsContainer');
        if(!c||!supabaseClient)return;
        c.innerHTML='<p style="color:var(--bark-soft);text-align:center;grid-column:1/-1;">Loading celebrations...</p>';
        try{
            const{data,error}=await supabaseClient.from('celebrations').select('*');
            if(error)throw error;
            if(!data||data.length===0){
                c.innerHTML='<p style="color:var(--bark-soft);text-align:center;grid-column:1/-1;">No upcoming celebrations listed right now.</p>';
                return;
            }
            const now=new Date();
            const currentMonth=now.getMonth()+1;
            ...
            const todayCelebs=data.filter(x=>x.month===currentMonth&&x.day===currentDay);
            if(todayCelebs.length>0){
                triggerConfetti();
                const badge=document.getElementById('celebrationBadge');
                const name=todayCelebs[0].name;
                const type=todayCelebs[0].type;
                badge.innerHTML=`🎉 Happy ${type}, ${name}!`;
                badge.style.display='flex';
            }
            
            const upcoming=data.filter(x=>{
                if(x.month===currentMonth&&x.day>=currentDay)return true;
                if(x.month===(currentMonth%12)+1)return true;
                return false;
            }).sort((a,b)=>(a.month*100+a.day)-(b.month*100+b.day));
            
            if(upcoming.length===0){
                c.innerHTML='<p style="color:var(--bark-soft);text-align:center;grid-column:1/-1;">No upcoming celebrations in the near future.</p>';
                return;
            }
            
            c.innerHTML=upcoming.map(ev=>{
                const icon=ev.type==='Birthday'?'🎂':'💍';
                return `<div class="celebration-card reveal in"><div class="icon">${icon}</div><div class="info"><h4>${ev.name}</h4><p>${ev.day} ${monthNames[ev.month-1]} • ${ev.type}</p></div></div>`;
            }).join('');
            
        }catch(e){
            c.innerHTML='<p style="color:var(--bark-soft);text-align:center;grid-column:1/-1;">Could not load celebrations.</p>';
        }
    }
    
    function triggerConfetti(){
        const container=document.getElementById('confettiContainer');
        const colors=['#C25528','#A87816','#A63A5A','#7CA890','#1AA37E','#F2C200'];
        for(let i=0;i<50;i++){
    ...
    async function loadAdminCelebrations(){
        const ac=document.getElementById('adminCelebContainer');
        if(!ac||!supabaseClient)return;
        ac.innerHTML='<p class="text-sm" style="color: var(--bark-soft);">Loading...</p>';
        try{
            const{data,error}=await supabaseClient.from('celebrations').select('*').order('month',{ascending:true}).order('day',{ascending:true});
            if(error)throw error;
            if(!data||data.length===0){
                ac.innerHTML='<p class="text-sm" style="color: var(--bark-soft);">No celebrations found.</p>';
                return;
            }
            ...
            ac.querySelectorAll('.del-celeb-btn').forEach(btn=>btn.addEventListener('click',async(e)=>{
                const id=e.target.dataset.id;
                try{
                    await supabaseClient.from('celebrations').delete().eq('id',id);
                    ToastModule.show('Celebration deleted!');
                    loadAdminCelebrations();
                    loadCelebrations();
                }catch(err){
                    ToastModule.show('Error deleting celebration.');
                }
            }));
        }catch(e){
            ac.innerHTML='<p class="text-sm" style="color: var(--terracotta);">Error loading celebrations.</p>';
        }
    }
    
    async function addCelebration(){
        const name=document.getElementById('newCelebName').value.trim();
        const type=document.getElementById('newCelebType').value;
        const month=parseInt(document.getElementById('newCelebMonth').value);
        const day=parseInt(document.getElementById('newCelebDay').value);
        
        if(!name||!day||day<1||day>31){
            ToastModule.show('Name and valid Day (1-31) are required.');
            return;
        }
        try{
            const{error}=await supabaseClient.from('celebrations').insert([{name:name,type:type,month:month,day:day}]);
            if(error)throw error;
            ToastModule.show('Celebration added!');
            document.getElementById('newCelebName').value='';
            document.getElementById('newCelebDay').value='';
            loadAdminCelebrations();
            loadCelebrations();
        }catch(err){
            ToastModule.show('Error adding celebration.');
        }
    }
    ...
    function init(){
        ...
        if(btn)btn.addEventListener('click',addCelebration);
    }
...
const TesterModule=(function(){
    ...
    function ss(s){
        ...
        safeSet('th-theme','light');safeSet('th-palette','nature');
        if(s==='auto'){const cs=SeasonalModule.getCurrentSeason();b.classList.add('season-'+cs);generateSeasonalBackground(cs)}
        else{b.classList.add('season-'+s);generateSeasonalBackground(s)}
        document.querySelectorAll('.tester-season-btn').forEach(b=>b.classList.remove('active'));
        event.target.classList.add('active');
        document.querySelectorAll('.theme-btn').forEach(e=>e.setAttribute('aria-pressed',e.dataset.theme==='light'))
    }
    
    function bc(){
        const inp=document.getElementById('broadcastInput'),msg=inp.value.trim();
        if(!msg)return;
        fetch('https://ntfy.sh/tinkers-hatch-live',{method:'POST',body:msg}).then(()=>{inp.value='';ToastModule.show('Message broadcasted successfully!')}).catch(err=>ToastModule.show('Error broadcasting message.'))
    }
    ...
    async function uploadPhoto(){
        ...
        if(error){ToastModule.show("Upload failed: "+error.message);}
        else{
            ...
            ac.querySelectorAll('.del-photo-btn').forEach(btn=>btn.addEventListener('click',async(e)=>{
                ...
                if(confirm('Are you sure you want to delete this photo?')){
                    try{
                        ...
                        renderPhotoAdmin();
                        if(typeof LightboxModule!=='undefined')LightboxModule.loadImages();
                    }catch(err){ToastModule.show('Error deleting photo.');}
                }
            }));
        }catch(err){ac.innerHTML='<p class="text-sm" style="color: var(--terracotta);">Error loading photos.</p>';}
    }
...
const EventsModule=(function(){
        ...
        try{
            ...
            if(printContainer){
                printContainer.innerHTML = `
                    <h1 style="font-family: 'Outfit', sans-serif; color: #152630; margin-bottom: 4px;">Tinkers Hatch</h1>
                    <h2 style="font-family: 'Outfit', sans-serif; color: #4A5C66; font-size: 1.2rem; margin-top: 0; margin-bottom: 24px;">Full Upcoming Events Schedule</h2>
                    <div style="display: flex; flex-direction: column; gap: 16px;">
                        ${data.map(ev => {
                            const d = new Date(ev.event_date).toLocaleDateString('en-GB', {weekday: 'long', day: 'numeric', month: 'long', year: 'numeric'});
                            return `
                                <div style="border: 1px solid #ccc; border-radius: 8px; padding: 16px; break-inside: avoid;">
                                    <div style="font-weight: 800; font-size: 1.1rem; color: #1AA37E; margin-bottom: 4px;">${ev.title}</div>
                                    <div style="font-weight: 700; font-size: 0.9rem; color: #4A5C66; margin-bottom: 8px;">${d}</div>
                                    <div style="font-size: 0.95rem; color: #152630;">${ev.description || 'No additional details.'}</div>
                                </div>
                            `;
                        }).join('')}
                    </div>
                    <p style="margin-top: 32px; font-size: 0.8rem; color: #999; text-align: center;">Generated from tinkershatch.co.uk on ${new Date().toLocaleDateString('en-GB')}</p>
                `;
            }
        }catch(e){
    ...
    async function loadAdminEvents(){
        ...
        try{
            ...
            if(error)throw error;
            if(!data||data.length===0){
                ac.innerHTML='<p class="text-sm" style="color: var(--bark-soft);">No events found.</p>';
                return;
            }
            ...
            ac.querySelectorAll('.del-event-btn').forEach(btn=>btn.addEventListener('click',async(e)=>{
                const id=e.target.dataset.id;
                try{
    ...
    async function addEvent(){
        ...
        if(!t||!d){
            ToastModule.show('Title and Date are required.');
            return;
        }
        try{
...
const VibeModule=(function(){
    async function loadVibe(){
        if(!supabaseClient) return;
        try{
            const{data,error}=await supabaseClient.from('site_settings').select('current_vibe').eq('id',1).single();
            if(error)throw error;
            applyVibe(data?.current_vibe || 'neutral');
        }catch(e){
            applyVibe('neutral'); // Fallback to neutral if database is asleep
        }
    }

    function applyVibe(vibe){
        const b=document.body;
        b.classList.remove('vibe-active','vibe-calm','vibe-neutral');
        b.classList.add('vibe-'+vibe);
        
        // Update admin buttons if dashboard is open
    ...
    async function setVibe(vibe){
        if(!supabaseClient) return;
        try{
            const{error}=await supabaseClient.from('site_settings').update({current_vibe:vibe}).eq('id',1);
            if(error)throw error;
            applyVibe(vibe);
            ToastModule.show(`Vibe set to: ${vibe.charAt(0).toUpperCase() + vibe.slice(1)}`);
        }catch(err){
            ToastModule.show('Error updating the vibe.');
        }
    }

    function init(){
        loadVibe();
        // Hook up admin buttons
        document.querySelectorAll('.vibe-btn').forEach(btn=>{
...
const GoldenHourModule=(function(){
    function init(){
        const now = new Date();
        const hour = now.getHours();

        // Golden Hour 1: 06:00 to 08:00 (Sunrise)
        // Golden Hour 2: 18:00 to 20:00 (Sunset)
...
document.addEventListener('DOMContentLoaded',()=>{
	HapticModule.init();BionicModule.init();NextSectionModule.init();
